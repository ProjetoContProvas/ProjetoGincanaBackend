...
public class Notas_CriterioController {
	...
	@GetMapping("/{id}")
	public Notas_CriterioModel findById(@PathVariable("id") Integer id) {
	...
	@PutMapping("/{id}")
	@ResponseStatus(HttpStatus.CREATED)
	public ResponseEntity<Notas_CriterioModel> update(@PathVariable("id") Integer id, @RequestBody Notas_CriterioModel notas_criterioModel) {
		var p = repository.findById(id);
		if (p.isPresent()) {
			var notas_criterio = p.get();
			if (notas_criterioModel.getNota_Criterio() != 0)
				notas_criterio.setNota_Criterio(notas_criterioModel.getNota_Criterio());
